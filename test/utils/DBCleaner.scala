...
object DBCleaner {
    ...
    def cleanUp():Unit = {
        ...
        val db = Database.forURL("jdbc:mysql://127.0.0.1:3306/rws_db?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC&useSSL=false", "root", "Welcome123")
        try {
            Await.result(
                db.run(
                    DBIO.seq(
